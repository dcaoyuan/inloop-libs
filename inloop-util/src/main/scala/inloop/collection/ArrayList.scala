 ...
 *
 *  serialver -classpath ~/myapps/scala/lib/scala-library.jar:./ inloop.collection.ArrayList
 *
...
final class ArrayList[A](
  _initialSize: Int, _elementClass: Option[Class[A]] = None)(implicit _m: ClassTag[A]) extends AbstractArrayList[A](_initialSize, _elementClass)(_m)
   ...
   */
  override def clone(): ArrayList[A] = new ArrayList[A](size) ++= this
...
abstract class AbstractArrayList[A](
  ...
  protected val elementClass: Option[Class[A]])(protected implicit val m: ClassTag[A]) extends AbstractBuffer[A]
    with Buffer[A]
